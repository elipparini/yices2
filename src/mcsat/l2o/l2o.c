...
void l2o_construct(l2o_t* l2o, l2o_mode_t mode, term_table_t* terms, jmp_buf* handler, plugin_t* nra) {
  ...
  l2o_evaluator_construct(l2o, &l2o->evaluator);
  l2o->tracer = NULL;
...
void l2o_destruct(l2o_t* l2o) {
  ...
  l2o_evaluator_destruct(&l2o->evaluator);
...
void collect_freevars(l2o_t* l2o, term_t t) {
  ...
  ivector_t variables, offset;
  init_ivector(&variables, 0);
  init_ivector(&offset, 0);

  while (stack.size > 0) {
    // Current term (unsigned)
    term_t current = unsigned_term(ivector_last(&stack));

    if(!int_hset_add(&l2o->terms_processed, current)) {
      // If already visited, continue
      ivector_pop(&stack);
      continue;
    }
    ...
    ivector_push(&offset, variables.size);
    ...
    {
      ivector_push(&variables, current);
...
void l2o_reset(l2o_t *l2o) {
  ...
  l2o_evaluator_reset(&l2o->evaluator);
}
